...
def on_message(ws, message):
	...
	if "payload" not in to_dict.keys() or "heartbeat" in to_dict.values():
		return

	sub_dict = to_dict["payload"]
	id = int(sub_dict["zone_id"])
	continent = continent_ids[id] # a string provided that zone_id matches key in continent_id
...
def websocket_thread():
	ws = websocket.WebSocketApp('wss://push.planetside2.com/streaming?environment=ps2&service-id=s:alerts', on_open=on_open, on_message=on_message)
	ws.run_forever()
